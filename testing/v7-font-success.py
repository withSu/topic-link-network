...
from typing import Dict, List, Any, Set, Tuple, Optional, TypedDict
import numpy as np
# 폰트 설치 후 matplotlib 캐시 갱신
...
# EnhancedMemoryStore 클래스 스텁 (원본에서 임포트하는 대신 필요한 부분만 정의)
class EnhancedMemoryStore:
    ...
    def add_memory(self, memory_data, related_memories=None):
    ...
    def search_by_triggers(self, query_text, triggers, top_k=5):
        # 간단한 검색 구현
        results = []
        for memory in self.memories:
            if any(trigger in memory.get('triggers', []) for trigger in triggers):
                results.append({
                    'memory': memory,
                    'similarity': 0.9,
                    'weighted_score': 0.9
...
# StateGraph 클래스 스텁 (LangGraph 의존성 제거)
class StateGraph:
...
# generate_default_response 함수 정의
...
# 필수 노드 함수 정의
def extract_triggers_node(state):
    # 간단한 구현
    user_input = state.get("user_input", "")
    # 기본 트리거 추출
    words = user_input.split()
    triggers = [word for word in words if len(word) > 2]
    
    state["triggers"] = triggers
    state["importance"] = 0.5
...
def store_memory_node(state):
    ...
    if memory_store:
        ...
        memory_store.add_memory(memory_data)
    return state

# AssociativeMemoryNetwork 클래스 (수정됨)
class AssociativeMemoryNetwork:
    ...
    def __init__(self, decay_factor=0.95, min_connection_strength=0.1):
        ...
        self.connection_types = {
            'semantic': 0.8,    # 의미적 유사성
            'procedural': 0.6,  # 절차적 관계
    ...
    def activate_concept(self, concept, related_concepts=None):
        """
        개념 활성화 및 연관 관계 학습
    ...
    def add_connection(self, from_concept, to_concept, connection_type='semantic', custom_strength=None):
        """두 개념 간 직접 연결 추가"""
        # 개념이 없으면 추가
    ...
    def _analyze_co_activation(self, concept):
                    ...
                    if activation['context']:
                        for ctx_concept in activation['context']:
                            if ctx_concept != concept:
                                co_activated.append(ctx_concept)
            else:
                break
    ...
    def _analyze_sequential_activation(self, concept):
    ...
    def _strengthen_connection(self, from_concept, to_concept, delta, connection_type='semantic'):
        """연결 강도 증가"""
        if self.graph.has_edge(from_concept, to_concept):
    ...
    def _update_co_occurrence_stats(self, concept1, concept2):
        """동시 발생 통계 업데이트"""
        # 각 노드의 동시 발생 통계 업데이트
    ...
    def get_related_concepts(self, concept, depth=2, min_strength=None):
        """
    ...
    def find_association_path(self, from_concept, to_concept, max_depth=4):
        """
    ...
    def visualize_network(self, central_concept=None, depth=2, save_path=None, show=True):
        """네트워크 시각화 (노드 텍스트 직접 렌더링)"""
        if central_concept:
            # 특정 개념 중심으로 서브그래프 생성
        ...
        for node in subgraph.nodes():
            ...
            node_sizes.append(500 + activation_count * 100)  # 크기 증가
        ...
        for font_name in korean_fonts:
            try:
                test_text = plt.text(0, 0, '한글', fontfamily=font_name)
                if hasattr(test_text, 'get_fontproperties'):
        ...
        # 엣지 레이블도 직접 그리기
        for (u, v), data in subgraph.edges.items():
            x = (pos[u][0] + pos[v][0]) / 2
            y = (pos[u][1] + pos[v][1]) / 2
            weight = data['weight']
            plt.text(x, y, f"{weight:.2f}",
                    horizontalalignment='center',
                    verticalalignment='center',
                    fontsize=10,
                    fontfamily=selected_font,
        ...
        plt.title(f"연관 기억 네트워크{f' - {central_concept} 중심' if central_concept else ''}", 
                fontsize=18, fontweight='bold', fontfamily=selected_font)
    ...
    def _get_most_activated_concepts(self, top_k):
        """가장 많이 활성화된 개념들"""
        concepts = []
    ...
    def _get_strongest_connections(self, top_k):
        """가장 강한 연결들"""
        connections = []
        for u, v, data in self.graph.edges(data=True):
...
def test_font_support():
    ...
    # 시스템 폰트 확인
    fonts = [f.name for f in fm.fontManager.ttflist if 'han' in f.name.lower() or 
            'gothic' in f.name.lower() or 'nanum' in f.name.lower() or 
            'cjk' in f.name.lower()]
    
    print(f"시스템에서 발견된 한글 폰트: {len(fonts)}개")
    for font in fonts[:10]:  # 처음 10개만 표시
        print(f"  - {font}")
    
    # 각 폰트 테스트
    working_fonts = []
    for font_name in ['NanumGothic', 'Malgun Gothic', 'Apple SD Gothic Neo', 'AppleGothic']:
        try:
            ...
            print(f"{font_name}: 실패 - {e}")
    
    print(f"\n작동하는 폰트: {working_fonts}")
    
    if working_fonts:
        print(f"선택된 폰트: {working_fonts[0]}")
...
class EnhancedAssociativeMemoryStore:
    ...
    def add_memory_with_associations(self, memory_data, related_concepts=None):
    ...
    def _extract_additional_concepts(self, memory_data):
        """메모리 데이터에서 추가 개념 추출"""
    ...
    def _build_associations(self, concepts, memory_data):
        """개념 간 연관 관계 구축"""
        # 감정적 연결
    ...
    def _group_by_semantics(self, concepts):
        """개념들을 의미별로 그룹화"""
    ...
    def recall_associations(self, query_concept, context=None):
        """
        개념과 관련된 기억 연쇄 회상
    ...
    def _create_concept_clusters(self, concepts):
        """연관된 개념들을 클러스터로 그룹화"""
    ...
    def strengthen_association_by_recall(self, concept, recalled_memory):
        """회상을 통한 연관 강화"""
    ...
    def visualize_association_web(self, central_concept, save_path=None):
        """연관 웹 시각화"""
    ...
    def _analyze_temporal_activation(self):
        """시간대별 개념 활성화 패턴 분석"""
...
def demonstrate_associative_memory():
    ...
    for query in queries:
        ...
        if recall_result['association_paths']:
            print("연관 경로:")
            for target, path in recall_result['association_paths'][:20]:
...
class AssociativeMemoryChatbot:
    ...
    def get_association_visualization(self, concept, save_path=None):
        """특정 개념의 연관 네트워크 시각화"""
        self.memory_system.visualize_association_web(concept, save_path)

# ----------------- LangGraph 노드 함수 개선 -----------------

def search_associative_memory_node(state):
    """연관 기억 기반 메모리 검색 노드"""
...
def generate_associative_response_node(state):
    """연관 기억을 활용한 응답 생성 노드"""
    user_input = state["user_input"]
...
def generate_associative_response(user_input, direct_memories, indirect_memories, association_context):
    """연관 기억을 활용한 응답 생성"""
    import random
...
def create_memory_chatbot_graph_with_associations():
    ...
    return memory_graph
# 메인 코드 시작 전에 전역 폰트 설정
def initialize_korean_font():
    """한글 폰트 초기화"""
    import matplotlib.pyplot as plt
    import matplotlib.font_manager as fm
    import platform
    ...
    if platform.system() == 'Darwin':  # macOS
        ...
        font_list = ['Malgun Gothic', 'Microsoft YaHei']
    else:  # Linux
        font_list = ['NanumGothic', 'Noto Sans CJK KR', 'DejaVu Sans']
    ...
    if selected_font:
        ...
        print("[폰트 설정] 한글 폰트를 찾을 수 없습니다. 기본 폰트를 사용합니다.")
...
def main():
    """연관 기억 챗봇 실행 예시"""
        # 한글 폰트 테스트
    selected_font = test_font_support()
